...
for (( i=0; i<n1; i++ )); do
	grep -P ^${inGroupArrayTmp[i]}'\t' Outputs/Comparative/Ortho_SNP_matrix_RAxML.phy &> /dev/null
	status=$?
	if [ "${status}" -ne 0 ]; then
...
for (( i=0; i<n2; i++ )); do
	grep -P ^${outGroupArrayTmp[i]}'\t' Outputs/Comparative/Ortho_SNP_matrix_RAxML.phy &> /dev/null
	status=$?
	if [ "${status}" -ne 0 ]; then
...
awk '{print $1, $1, $2, $3, $4, $5}' temp.col > genomes
sort -d genomes > genomes.mrg
cp ${PBS_O_WORKDIR}/Outputs/Comparative/Ortho_SNP_matrix_RAxML.phy Ortho_SNP_matrix_RAxML.phy
...
awk '{print $1}' genomes | while read f; do grep -P ^$f'\t' 	Ortho_SNP_matrix_RAxML.phy > $f.SNPs; done; 
for f in *.SNPs; do
	awk '{print $2}' $f > $f.tmp;
done;
for f in *.SNPs.tmp; do
