...
if [ "${status}" -ne 0 ]; then
	...
	echo -e "Completed:\n${2}" >> "${PBS_O_WORKDIR}/logs/Align_SNP_indel.cmds"
fi
}
...
if [ "${tech}" == Illumina -o "${tech}" == Illumina_old ]; then
	...
	if [ "${tech}" == Illumina -a "${pairing}" == SE ]; then
		if [ ! -s "${SAI1}" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
			log_eval ${PBS_O_WORKDIR} "${BWA} aln ${REF_FILE} ${READ1_FILE} -t ${CPUs} -f ${SAI1}"
		fi
	fi
	if [ "${tech}" == Illumina_old -a "${pairing}" == PE ]; then
		if [ ! -s "${SAI1}" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
			log_eval ${PBS_O_WORKDIR} "${BWA} aln -I ${REF_FILE} ${READ1_FILE} -t ${CPUs} -f ${SAI1} && ${BWA} aln -I ${REF_FILE} ${READ2_FILE} -t ${CPUs} -f ${SAI2}"
		fi
	fi
	if [ "${tech}" == Illumina_old -a "${pairing}" == SE ]; then
		if [ ! -s "${SAI1}" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
			log_eval ${PBS_O_WORKDIR} "${BWA} aln -I ${REF_FILE} ${READ1_FILE} -t ${CPUs} -f ${SAI1}"
		fi
	fi
	if [ "${pairing}" == PE ]; then
		if [ ! -s "${BAM_UNIQUE_FILE}.bam" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
			...
			log_eval ${PBS_O_WORKDIR} "${SAMTOOLS} view -@ ${CPUs} -h -b -S -q 1 ${SAM} | ${SAMTOOLS} sort -@ ${CPUs} -o ${BAM_UNIQUE_FILE}.bam -"
		fi
	fi
	if [ "${pairing}" == SE ]; then 
		if [ ! -s "${BAM_UNIQUE_FILE}.bam" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
	...
	if [ ! -s "${BAM_UNIQUE_INDEX_FILE}" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
		log_eval ${PBS_O_WORKDIR} "${SAMTOOLS} index -@ ${CPUs} ${BAM_UNIQUE_FILE}.bam"
	fi
fi
...
if [ "${tech}" == PGM -o "${tech}" == 454 -a "${pairing}" == SE ]; then
	if [ ! -s "${SAM}" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
		log_eval ${PBS_O_WORKDIR} "${BWA} bwasw -t ${CPUs} ${REF_FILE} ${READ1_FILE} > ${SAM}"
	fi
	if [ ! -s "${BAM}" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
		log_eval ${PBS_O_WORKDIR} "${SAMTOOLS} view -@ ${CPUs} -h -b -S -q 1 ${SAM} | ${SAMTOOLS} sort -@ ${CPUs} -o ${BAM}.bam -"
	fi
	if [ ! -s "${BAM_UNIQUE_FILE}.bam" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
	...
	if [ ! -s "${BAM_INDEX_FILE}.bam" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
...
if [ ! -s "${UNMAPPED}.bam" -a -s "${SAM}" ]; then
	...
	log_eval ${PBS_O_WORKDIR} "${SAMTOOLS} merge -@ ${CPUs} -u - ${TMP_BAM1} ${TMP_BAM2} ${TMP_BAM3} | ${SAMTOOLS} sort -@ ${CPUs} -n -o ${UNMAPPED} -"
fi
### cleanup ###
...
if [ ! -s "${GATK_TARGET_FILE}" -a ! -s "${GATK_REALIGNED_BAM}" ]; then
	log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -T RealignerTargetCreator -R ${REF_FASTA} -o ${GATK_TARGET_FILE} -I ${BAM_DUPLICATESREM_FILE}"
fi
if [ ! -s "${GATK_REALIGNED_BAM}" ]; then
	log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -T IndelRealigner -I ${BAM_DUPLICATESREM_FILE} -R ${REF_FASTA} -targetIntervals ${GATK_TARGET_FILE} -o ${GATK_REALIGNED_BAM}"
fi
### cleanup ###
...
if [ "${tech}" == PGM ]; then
	if [ ! -s "${GATK_DEPTH_FILE}" ]; then
		log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -T DepthOfCoverage -mmq 30 -R ${REF_FASTA} -rf BadCigar -omitBaseOutput -o ${seq_path}/${seq}/unique/${seq} -I ${GATK_REALIGNED_BAM}"
	fi
else
	if [ ! -s "${GATK_DEPTH_FILE}" ]; then
		log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -T DepthOfCoverage -mmq 30 -R ${REF_FASTA} -omitBaseOutput -o ${seq_path}/${seq}/unique/${seq} -I ${GATK_REALIGNED_BAM}"
	fi
fi
...
if [ "${tech}" == PGM ]; then
	if [ ! -s "${GATK_RAW_SNPS_FILE}" -a ! -s "${seq_path}/Outputs/SNPs_indels_PASS/${seq}.snps.PASS.vcf" ]; then
		log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -glm SNP -R ${REF_FASTA} -T UnifiedGenotyper -rf BadCigar -I ${GATK_REALIGNED_BAM} -o ${GATK_RAW_SNPS_FILE} -ploidy 1"
	fi
	if [ ! -s "${GATK_RAW_INDELS_FILE}" -a ! -s "${seq_path}/Outputs/SNPs_indels_PASS/${seq}.indels.PASS.vcf" ]; then
		log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -glm INDEL -R ${REF_FASTA} -T UnifiedGenotyper -rf BadCigar -I ${GATK_REALIGNED_BAM} -o ${GATK_RAW_INDELS_FILE} -ploidy 1"
	fi
else
	if [ ! -s "${GATK_RAW_SNPS_FILE}" -a ! -s "${seq_path}/Outputs/SNPs_indels_PASS/${seq}.snps.PASS.vcf" ]; then
		log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -glm SNP -R ${REF_FASTA} -T UnifiedGenotyper -I ${GATK_REALIGNED_BAM} -o ${GATK_RAW_SNPS_FILE} -ploidy 1"
	fi
	if [ ! -s "${GATK_RAW_INDELS_FILE}" -a ! -s "${seq_path}/Outputs/SNPs_indels_PASS/${seq}.indels.PASS.vcf" ]; then
		log_eval ${PBS_O_WORKDIR} "${JAVA} ${SET_VAR} -Djava.io.tmpdir=${PBS_O_WORKDIR}/tmp ${GATK} -glm INDEL -R ${REF_FASTA} -T UnifiedGenotyper -I ${GATK_REALIGNED_BAM} -o ${GATK_RAW_INDELS_FILE} -ploidy 1"
	fi
fi
...
if [ ! -s "${BED_COV_FILE}" -a ! -s "${seq_path}/BEDcov/${seq}.bedcov" ]; then
