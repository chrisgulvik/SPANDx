...
source "${SCRIPTPATH}"/scheduler.config  #enables multiple threads with CPUs var
...
if [ "${status}" -ne 0 ]; then
	...
	echo -e "Completed:\n$2" >> "${PBS_O_WORKDIR}/logs/SNP_matrix.cmds"
fi
}
...
if [ ! -s "${PBS_O_WORKDIR}/Phylo/out/out.vcf" ]; then
	echo "${BGZIP}" ../../BGZIP.ZZZ
	echo "${CPUs}" ../../CPUS.ZZZ
	log_eval ${PBS_O_WORKDIR}/Phylo/out "for f in *.clean.vcf; do ${BGZIP} -@ ${CPUs} -c \$f > \${f}.gz; done;"
	rm *.clean.vcf
...
if [ -s "${PBS_O_WORKDIR}/Phylo/out/merge.012.indv" -a ! -s "${PBS_O_WORKDIR}/Phylo/out/merge.012.indv.trans" ]; then
...
if [ ! -s "${PBS_O_WORKDIR}/Phylo/indels/out/out.vcf" -a "$indel_merge" == yes ]; then
	log_eval ${PBS_O_WORKDIR}/Phylo/indels/out "for f in *.clean.vcf; do ${BGZIP} -@ ${CPUs} -c \$f > \${f}.gz; done;" 
	rm *.clean.vcf
...
if [ -s "${PBS_O_WORKDIR}/Phylo/indels/out/merge.012.indv" -a ! -s "${PBS_O_WORKDIR}/Phylo/indels/out/merge.012.indv.trans" ]; then
...
if [ ! -s "${PBS_O_WORKDIR}/Phylo/out/Phylo_RAxML.nex" -a ! -s "${PBS_O_WORKDIR}/Outputs/Comparative/Ortho_SNP_matrix_RAxML.phy" ]; then 
	...
	echo -e "$y $x\n$taxa_and_grid" > Ortho_SNP_matrix_RAxML.phy
	mv ${PBS_O_WORKDIR}/Phylo/out/Ortho_SNP_matrix_RAxML.phy ${PBS_O_WORKDIR}/Outputs/Comparative/Ortho_SNP_matrix_RAxML.phy
fi
...
if [ "$tri_tetra_allelic" == "yes" ]; then
	...
	cat clean.out.tri_allelic.vcf | cut -d ' ' -f 10 - | sed 's/ /\t/g' > calls
...
if [ ! -s "${PBS_O_WORKDIR}/Outputs/Comparative/All_SNPs_annotated.txt" -a "$annotate" == yes ]; then
	...
	SnpEff_version=`($JAVA $SET_VAR $SNPEFF -h 2>&1 | head -n 1 |awk '{print $4}' | awk '{print substr($1,0,3)}' | bc)`
	if [ "$(echo $SnpEff_version '>=' 4.1 | bc -l)" -eq 1 ]; then
		...
		log_eval ${PBS_O_WORKDIR}/Phylo/annotated "$JAVA $SET_VAR $SNPEFF eff -t ${CPUs} -no-downstream -no-intergenic -ud 100 -formatEff -v $variant_genome ${PBS_O_WORKDIR}/Phylo/annotated/out.vcf > ${PBS_O_WORKDIR}/Phylo/annotated/out.annotated.vcf"
	else
		echo -e "Version of snpEff is less than 4.1\n"
		log_eval ${PBS_O_WORKDIR}/Phylo/annotated "$JAVA $SET_VAR $SNPEFF eff -t ${CPUs} -no-downstream -no-intergenic -ud 100 -v $variant_genome ${PBS_O_WORKDIR}/Phylo/annotated/out.vcf > ${PBS_O_WORKDIR}/Phylo/annotated/out.annotated.vcf"
	fi	
	rm snpEff_genes.txt
...
if [ ! -s "${PBS_O_WORKDIR}/Outputs/Comparative/All_indels_annotated.txt" -a "${annotate}" == yes -a "${indel_merge}" == yes ]; then
	...
	if [ "$(echo $SnpEff_version '>=' 4.1 | bc -l)" -eq 1 ]; then
		echo -e "Version of snpEff is 4.1 or greater\n"
		echo -e "Version of SnpEff is $SnpEff_version"
		log_eval ${PBS_O_WORKDIR}/Phylo/annotated "$JAVA $SET_VAR $SNPEFF eff -t ${CPUs} -no-downstream -no-intergenic -ud 100 -formatEff -v $variant_genome ${PBS_O_WORKDIR}/Phylo/annotated/out_indels.vcf > ${PBS_O_WORKDIR}/Phylo/annotated/out_indels.annotated.vcf"
	else
		echo -e "Version of snpEff is less than 4.1\n"
		log_eval ${PBS_O_WORKDIR}/Phylo/annotated "$JAVA $SET_VAR $SNPEFF eff -t ${CPUs} -no-downstream -no-intergenic -ud 100 -v $variant_genome ${PBS_O_WORKDIR}/Phylo/annotated/out_indels.vcf > ${PBS_O_WORKDIR}/Phylo/annotated/out_indels.annotated.vcf"
	fi	
	rm snpEff_genes.txt
	rm snpEff_summary.html
